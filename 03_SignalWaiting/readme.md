...
Pause 상태의 워커 스레드는 CPU를 계속 쓰면서 반복 검사하는 것이 아니라,
조건이 바뀔 때까지 대기 상태로 들어가도록 구현합니다.

Running 워커 스레드가 1초마다 `Tick` / `Tock`을 번갈아 출력합니다.
...
스레드는 프로세스 안에서 CPU 명령어를 실행하는 흐름입니다.
하지만 CPU는 한 순간에 하나의 명령어 흐름만 실행할 수 있습니다.
따라서 여러 스레드가 동시에 존재하더라도, 실제로 어떤 스레드가 CPU를 사용할지는 운영체제가 조정해야 합니다.

이 역할을 하는 구성 요소가 OS 스케줄러(scheduler)입니다.
스케줄러는 각 스레드의 실행 상태를 보고 다음에 CPU를 사용할 스레드를 선택합니다.
그래서 이 문서에서 말하는 `Ready`, `Running`, `Blocked / Waiting` 같은 상태는
운영체제가 스레드를 스케줄링하고 대기시키기 위해 관리하는 실행 상태라고 볼 수 있습니다.

스케줄러가 개입하는 대표적인 순간은 아래와 같습니다.

- 타이머 인터럽트: 실행 중인 스레드가 퀀텀(Time Quantum)을 모두 사용했을 때
- I/O 인터럽트: 키보드, 마우스, 디스크, 네트워크 작업이 완료되었을 때
- 시스템 콜: 스레드가 `Sleep`, 대기, I/O 같은 OS 기능을 요청할 때
- 동기화 신호: Event, Mutex, Condition Variable 등의 상태가 바뀌었을 때

#### 스레드 상태

스레드 상태는 스케줄러가 CPU를 줄 수 있는 스레드인지, 아니면 아직 어떤 조건을 기다려야 하는 스레드인지 구분하는 기준입니다.
...
`Ready` 상태의 스레드는 실행 조건은 만족했지만 아직 CPU를 받지 못한 상태입니다.
즉, CPU만 배정되면 바로 실행될 수 있는 스케줄러의 실행 후보입니다.

`Running` 상태는 실제 CPU 코어에서 실행 중인 상태입니다.
스케줄러가 `Ready` 상태의 스레드 중 하나를 선택해 CPU에서 실행시키면 `Ready -> Running` 전환이 발생합니다.
이 전환을 **dispatch**라고 합니다.

`Blocked / Waiting` 상태는 대기 조건이 풀리지 않아 CPU를 받아도 아직 실행을 계속할 수 없는 상태입니다.
예를 들어 `Sleep`, I/O 대기, `condition_variable::wait`, `WaitForMultipleObjects`처럼 조건이 만족될 때까지 기다리는 API를 호출하면,
스레드는 CPU를 계속 붙잡고 있지 않고 `Blocked / Waiting` 상태로 이동합니다.
시간이 지나거나 I/O가 완료되거나 동기화 객체가 신호 상태가 되면 대기 조건이 풀리고,
스레드는 다시 `Ready` 상태가 되어 스케줄러의 선택을 기다립니다.

실행 중이던 스레드가 퀀텀을 모두 사용하거나 더 높은 우선순위의 스레드에게 CPU를 넘겨야 하면,
`Running` 상태에서 다시 `Ready` 상태로 돌아갈 수 있습니다.
이 전환을 **preempt**라고 합니다.

#### 문맥 교환(Context Switching)

스케줄러가 CPU에서 실행할 스레드를 바꾸면, 운영체제는 현재 스레드의 실행 상태를 저장하고 다음 스레드의 실행 상태를 복원해야 합니다.
이 작업을 **문맥 교환(context switch)** 이라고 합니다.

문맥 교환에서 저장하고 복원하는 대표적인 정보는 CPU 레지스터, 스택 포인터, 명령어 포인터처럼
스레드가 나중에 이어서 실행되기 위해 필요한 실행 문맥입니다.
